...
void ClusterLossLayer<Dtype>::Reshape(
	...
	N_ = bottom[0]->shape(0);
	K_ = bottom[0]->count(1);
	if (bottom[1]->count() != N_)
		CHECK_EQ(bottom[1]->count(), K_) << "Bottom[1] should either be tha label (N*1) or the prior (K*1).";
...
void ClusterLossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
	...
	if (bottom[1]->count() == K_) {
		...
		for (int n = 0; n < N_; ++n) {
			...
			loss -= log_posterior_data[n * K_ + label_value];
		}
	}
...
void ClusterLossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
	...
	if (bottom[1]->count() == K_) {
		...
		if (propagate_down[0]) {
			Dtype* log_posterior_diff = bottom[0]->mutable_cpu_diff();
			caffe_set<Dtype>(bottom[0]->count(), Dtype(0), log_posterior_diff);
			const Dtype* label = bottom[1]->cpu_data();
			for (int n = 0; n < N_; ++n) {
				const int label_value = static_cast<int>(label[n]);
				log_posterior_diff[n * K_ + label_value] = -scale;
			}
		}
		if (propagate_down[1])
